...
export default function ChallengeBox({ question }: ChallengeBoxProps) {
  ...
  const { user, updateUserScore, isTeacher } = useAuth();
  const isAdmin = user?.role === 'admin';
  const canViewAnswers = isTeacher || isAdmin;

  const questionKey = String(question.id);
  const answeredData = user?.answeredQuestions[questionKey];
  ...
  const handleSubmit = () => {
    ...
    updateUserScore(question.id, question.points, correct);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-lg font-bold">Q{question.id}</span>
          ...
          <div className="space-y-6 py-4">
            ...
            {canViewAnswers && (
              <div className="p-4 rounded-xl bg-accent/10 text-accent text-center">
                <p className="font-semibold">👁️ {isAdmin ? 'Admin' : 'Teacher'} Mode: Correct answer is highlighted</p>
              </div>
            )}
            ...
            {isAnswered && !isTeacher && (
              <div className={cn(
                ...
                {isCorrect ? (
                  ...
                  <p className="font-semibold">❌ Incorrect. The correct answer was: {question.options[question.correctAnswer]}</p>
                )}
              </div>
            )}
          </div>
